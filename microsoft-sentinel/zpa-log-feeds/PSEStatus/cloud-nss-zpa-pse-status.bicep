/*
This creates the DCE, DCR, and ZPA_CL custom table needed to ingest ZPA PSE (Service Edge)
Status logs into Sentinel via Zscaler's Cloud NSS Log Ingestion API path.

The DCR transform reconstructs the incoming Format-2 JSON into the ZPA_CL.Message column so
that the existing ZPAPSEStatus() KQL function (which expects Message to contain the full
event JSON) works without modification. This means the same function and workbooks serve
...
2. Deploy:
    ...
    az stack group create --name cloud-nss-zpa-pse-status --resource-group <rg> --template-file ./cloud-nss-zpa-pse-status.bicep --deny-settings-mode 'none' --action-on-unmanage deleteResources

   Or with .bicepparam:

    az stack group create --name cloud-nss-zpa-pse-status --resource-group <rg> --parameters cloud-nss-zpa-pse-status.bicepparam --deny-settings-mode 'none' --action-on-unmanage deleteResources

3. Grant the App Registration 'Monitoring Metrics Publisher' on the created DCR.
4. Retrieve the ingest URL:

    az stack group show -g <rg> -n cloud-nss-zpa-pse-status --query outputs.api_url

5. Configure the ZPA PSE Status Cloud NSS feed in the Zscaler portal using the .fof
...
output api_url string = '${dce.properties.logsIngestion.endpoint}/dataCollectionRules/${dcr.properties.immutableId}/streams/Custom-ZPAPSEStatus_CL?api-version=2023-01-01'
