/*
This creates the DCE, DCR, and ZPA_CL custom table needed to ingest ZPA App Connector Status
logs into Sentinel via Zscaler's Cloud NSS Log Ingestion API path.

The DCR transform reconstructs the incoming Format-2 JSON into the ZPA_CL.Message column so
that the existing ZPAAppConnectorStatus() KQL function (which expects Message to contain the
full event JSON) works without modification. This means the same function and workbooks serve
both the AMA/syslog VM pipeline and the Log Ingestion API pipeline.

Deployment steps:
1. Create an App Registration as per the Deployment Guide.
2. Deploy:

    az stack group create --name cloud-nss-zpa-appconnectorstatus --resource-group <rg> --template-file ./cloud-nss-zpa-appconnectorstatus.bicep --deny-settings-mode 'none' --action-on-unmanage deleteResources

   Or with .bicepparam:

    az stack group create --name cloud-nss-zpa-appconnectorstatus --resource-group <rg> --parameters cloud-nss-zpa-appconnectorstatus.bicepparam --deny-settings-mode 'none' --action-on-unmanage deleteResources

3. Grant the App Registration 'Monitoring Metrics Publisher' on the created DCR.
4. Retrieve the ingest URL:

    az stack group show -g <rg> -n cloud-nss-zpa-appconnectorstatus --query outputs.api_url

5. Configure the ZPA App Connector Status Cloud NSS feed in the Zscaler portal using the .fof
...
resource dcr 'Microsoft.Insights/dataCollectionRules@2022-06-01' = {
  ...
  properties: {
    ...
    streamDeclarations: {
      'Custom-ZPAAppConnectorStatus_CL': {
        columns: [
...
output api_url string = '${dce.properties.logsIngestion.endpoint}/dataCollectionRules/${dcr.properties.immutableId}/streams/Custom-ZPAAppConnectorStatus_CL?api-version=2023-01-01'
