/*
This creates the DCE, DCR, and ZPA_CL custom table needed to ingest ZPA User Status logs
...
that the existing ZPAUserStatus() KQL function (which expects Message to contain the full
...
2. Deploy:
    ...
    az stack group create --name cloud-nss-zpa-userstatus --resource-group <rg> --template-file ./cloud-nss-zpa-userstatus.bicep --deny-settings-mode 'none' --action-on-unmanage deleteResources

   Or with .bicepparam:

    az stack group create --name cloud-nss-zpa-userstatus --resource-group <rg> --parameters cloud-nss-zpa-userstatus.bicepparam --deny-settings-mode 'none' --action-on-unmanage deleteResources

3. Grant the App Registration 'Monitoring Metrics Publisher' on the created DCR.
4. Retrieve the ingest URL:

    az stack group show -g <rg> -n cloud-nss-zpa-userstatus --query outputs.api_url

5. Configure the ZPA User Status Cloud NSS feed in the Zscaler portal using the .fof
    ...
    dataFlows: [
      {
        ...
        // ZPAUserStatus() function (which parse_json's Message) keeps working identically
        // across the VM/syslog and Log Ingestion API pipelines. TimeGenerated derives from
        // event.LogTimestamp — Cloud NSS emits this as Unix epoch SECONDS (10 digits).
...
output api_url string = '${dce.properties.logsIngestion.endpoint}/dataCollectionRules/${dcr.properties.immutableId}/streams/Custom-ZPAUserStatus_CL?api-version=2023-01-01'
