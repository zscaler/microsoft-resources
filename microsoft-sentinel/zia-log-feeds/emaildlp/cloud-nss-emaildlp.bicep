...
2. Run this script from Azure CLI using the following command:
    ...
    az stack group create --name cloud-nss-emaildlp --resource-group rg-sentinel-cloud-emaildlp --template-file ./cloud-nss-emaildlp.bicep --deny-settings-mode 'none'
  ...
  Alternatively, you can specify these parameters ahead of time by populating the cloud-nss-emaildlp.bicepparams file and running the following command in Azure CLI to deploy:

    az stack group create --name cloud-nss-emaildlp --resource-group <resource group containing your log analytics workspace> --parameters cloud-nss-emaildlp.bicepparam --deny-settings-mode 'none' --action-on-unmanage deleteResources
...
4. Configure your Cloud NSS feed in the Zscaler Portal. You can retrieve the feed API URL using the following command in Azure CLI:
    ...
    az stack group show -g rg-sentinel-cloud-emaildlp -n cloud-nss-emaildlp --query outputs.api_url

5. If you ever need to delete the deployment, you can run the following command from Azure CLI:

    az stack group delete --name cloud-nss-emaildlp --resource-group rg-sentinel-cloud-emaildlp --delete-resources
...
output api_url string = '${dce.properties.logsIngestion.endpoint}/dataCollectionRules/${dcr.properties.immutableId}/streams/Custom-nss_emaildlp_CL?api-version=2021-11-01-preview'
