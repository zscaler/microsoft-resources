...
2. Run this script from Azure CLI using the following command:
    ...
    az stack group create --name cloud-nss-casbfilesharing --resource-group <resource group containing your log analytics workspace> --template-file ./cloud-nss-casbfilesharing.bicep --deny-settings-mode 'none' --action-on-unmanage deleteResources
  ...
  Alternatively, you can specify these parameters ahead of time by populating the cloud-nss-casbfilesharing.bicepparams file and running the following command in Azure CLI to deploy:
    ...
    az stack group create --name cloud-nss-casbfilesharing --resource-group <resource group containing your log analytics workspace> --parameters cloud-nss-casbfilesharing.bicepparam --deny-settings-mode 'none' --action-on-unmanage deleteResources
...
4. Configure your Cloud NSS feed in the Zscaler Portal. You can retrieve the feed API URL using the following command in Azure CLI:
    ...
    az stack group show -g <resource group containing your log analytics workspace> -n cloud-nss-casbfilesharing --query outputs.api_url

5. If you ever need to delete the deployment, you can run the following command from Azure CLI:

    az stack group delete --name cloud-nss-casbfilesharing --resource-group <resource group containing your log analytics workspace> --delete-resources
