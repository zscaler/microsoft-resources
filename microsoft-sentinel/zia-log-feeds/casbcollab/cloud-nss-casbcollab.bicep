...
2. Run this script from Azure CLI using the following command:
    ...
    az stack group create --name cloud-nss-casbcollab --resource-group rg-sentinel-cloud-casbcollab --template-file ./cloud-nss-casbcollab.bicep --deny-settings-mode 'none'
  ...
  Alternatively, you can specify these parameters ahead of time by populating the cloud-nss-casbcollab.bicepparams file and running the following command in Azure CLI to deploy:
    ...
    az stack group create --name cloud-nss-casbcollab --resource-group rg-sentinel-cloud-casbcollab --parameters cloud-nss-casbcollab.bicepparam --deny-settings-mode 'none'
...
4. Configure your Cloud NSS feed in the Zscaler Portal. You can retrieve the feed API URL using the following command in Azure CLI:
    ...
    az stack group show -g rg-sentinel-cloud-casbcollab -n cloud-nss-casbcollab --query outputs.api_url

5. If you ever need to delete the deployment, you can run the following command from Azure CLI:

    az stack group delete --name cloud-nss-casbcollab --resource-group rg-sentinel-cloud-casbcollab --delete-resources
