...
2. Run this script from Azure CLI using the following command:
    ...
    az stack group create --name cloud-nss-casbcrm --resource-group <resource group containing your log analytics workspace> --template-file ./cloud-nss-casbcrm.bicep --deny-settings-mode 'none' --action-on-unmanage deleteResources
  ...
  Alternatively, you can specify these parameters ahead of time by populating the cloud-nss-casbcrm.bicepparams file and running the following command in Azure CLI to deploy:

    az stack group create --name cloud-nss-casbcrm --resource-group <resource group containing your log analytics workspace> --parameters cloud-nss-casbcrm.bicepparam --deny-settings-mode 'none' --action-on-unmanage deleteResources
...
4. Configure your Cloud NSS feed in the Zscaler Portal. You can retrieve the feed API URL using the following command in Azure CLI:
    ...
    az stack group show -g <resource group containing your log analytics workspace> -n cloud-nss-casbcrm --query outputs.api_url

5. If you ever need to delete the deployment, you can run the following command from Azure CLI:

    az stack group delete --name cloud-nss-casbcrm --resource-group <resource group containing your log analytics workspace> --delete-resources
...
output api_url string = '${dce.properties.logsIngestion.endpoint}/dataCollectionRules/${dcr.properties.immutableId}/streams/Custom-nss_casbcrm_CL?api-version=2021-11-01-preview'
